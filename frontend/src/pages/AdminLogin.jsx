...
const AdminLogin = () => {
    ...
    const handleRedirectResult = async () => {
      try {
        ...
        if (result) {
          ...
          console.log('👤 Admin display name:', result.user.displayName);
          ...
          if (window.location.pathname === '/__/auth/handler') {
            console.log('🏠 Redirecting from auth handler to admin login...');
  ...
  const checkAdminExists = async (email) => {
    try {
      ...
      if (error.response?.status === 404) {
        console.log('ℹ️ Admin does not exist in database');
        return { exists: false };
      }
  ...
  const handleSuccessfulGoogleAuth = async (user) => {
    try {
      console.log('🎉 Google authentication successful for admin:', user.email);
      ...
      if (adminCheck.exists) {
        console.log('👤 Existing admin found, navigating to admin dashboard');
        toast.success(`Welcome back, ${user.displayName || user.email}!`);
        navigate('/admin-dashboard');
      } else {
        console.log('🆕 New admin, navigating to admin company setup');
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      console.log('Current auth state:', auth.currentUser ? 'Admin logged in' : 'No admin');
      ...
      switch (error.code) {
        ...
        case 'auth/unauthorized-domain':
          ...
          break;
        case 'auth/invalid-api-key':
          toast.error('Invalid Firebase API key. Please check Firebase Console.');
          break;
        ...
        <div className="w-full max-w-md space-y-8">
          <div>
            ...
            <p className="mt-2 text-center text-sm text-[var(--subtle-text-color)]">
              ...
              <Link className="font-medium text-[var(--primary-color)] hover:text-blue-500 ml-1" to="/admin-signup"> Sign up </Link>
            </p>
          </div>
