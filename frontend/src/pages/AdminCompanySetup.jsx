...
const AdminCompanySetup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Welcome! Your admin account has been set up successfully.');

      // Store admin user data in localStorage
      const userData = {
        name: name.trim(),
        email: email.trim(),
        uid: response.data?.uid || `admin_${Date.now()}`,
        photoURL: null
      };
      ...
      if (error.response?.data?.message) {
        ...
        toast.error('Failed to set up your admin account. Please try again.');
      }
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-[var(--subtle-text-color)]">
              Please provide your company information to complete your admin account setup
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <button
                type="submit"
                disabled={isLoading}
                className="group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-[var(--primary-color)] hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 focus:ring-offset-[var(--background-color)] disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                {isLoading ? (
